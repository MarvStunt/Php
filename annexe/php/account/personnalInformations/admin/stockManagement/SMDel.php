...
$sql = $pdo->query("DELETE FROM gestionStock WHERE id_produit = $id_produit");

// Si la requete c'est bien effectué on redirige vers la page de gestion du stock avec un message de succes
if ($sql) {
   header("Location: SM.php?success=Produit supprimé avec succès");
   exit();
} else {
   header("Location: SM.php?error=Erreur lors de la suppression du produit");
   exit();
}
