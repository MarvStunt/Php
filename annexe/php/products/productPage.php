   ...
   <main>
      ...
      <div id="product-grid">
         ...
         if (!empty($_GET["reference"])) {
            ...
            // Récupération des produits
            $requeteProd = $pdo->query("SELECT (referenceProduit,image, iconePresentation, titreProduit, prixPublic, prixAchat, descriptif, nom, auteur) FROM produit p, gestionStock g WHERE g.id_produit=p.id_produit AND g.quantite<10");
            $products = $requeteProd->fetchAll(PDO::FETCH_ASSOC);
         } else if (!empty($_GET["New"])) {

            // Récupération des produits
            $requeteProd = $pdo->query("SELECT (referenceProduit,image, iconePresentation, titreProduit, prixPublic, prixAchat, descriptif, nom, auteur) FROM produit p, gestionStock g WHERE g.id_produit=p.id_produit AND g.dateModif >= DATE_SUB(CURRENT_DATE(), INTERVAL 15 DAY)");
            $products = $requeteProd->fetchAll(PDO::FETCH_ASSOC);
